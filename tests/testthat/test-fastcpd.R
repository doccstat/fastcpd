...
testthat::test_that("linear regression with multi-dimensional responses", {
  set.seed(1)
  n <- 300
  p <- 3
  y_count <- 2
...
testthat::test_that("multivariate mean change", {
  set.seed(1)
  p <- 3
  data <- rbind(
    mvtnorm::rmvnorm(300, mean = rep(0, p), sigma = diag(100, p)),
...
testthat::test_that("multivariate mean or variance change", {
  set.seed(1)
  p <- 4
  data <- rbind.data.frame(
    mvtnorm::rmvnorm(300, mean = rep(0, p), sigma = diag(1, p)),
  ...
  testthat::expect_equal(
    meanvar_loss_result@cp_set, c(300, 696, 1000, 1300, 1697)
  )
})
