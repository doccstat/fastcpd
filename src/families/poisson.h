...
struct PoissonFamily : BaseFamily {
  ...
  static arma::mat CreateDataC(arma::mat const& data,
    ...
    arma::mat log_factorial_prefix(1, data.n_rows + 1);
    log_factorial_prefix(0, 0) = 0.0;
    for (arma::uword row = 0; row < data.n_rows; row++) {
      log_factorial_prefix(0, row + 1) =
          log_factorial_prefix(0, row) + std::lgamma(data(row, 0) + 1.0);
    }
    return log_factorial_prefix;
  }
  ...
  static void CreateSenParameters(Solver* solver) {
    ...
    solver->hessian_.slice(0) =
        TailOuter(solver, 0,
                  std::exp(DotTail(solver, 0, solver->coefficients_.col(0))));
  }
  ...
  static void UpdateSenParameters(Solver* solver) {
    ...
    arma::mat hessian_new =
        TailOuter(solver, solver->t - 1,
                  std::exp(DotTail(solver, solver->t - 1, coef_add)));
  ...
  static double GetNllSen(Solver* solver, unsigned int const segment_start,
    ...
    double cost = solver->data_c_ptr_[segment_end + 1] -
                  solver->data_c_ptr_[segment_start];
    for (unsigned int row = segment_start; row <= segment_end; row++) {
      double const eta = DotTail(solver, row, theta);
      cost += -solver->data_(row, 0) * eta + std::exp(eta);
    }
    return cost;
  }
  ...
  static arma::colvec GetGradient(Solver* solver,
                                  ...
                                  unsigned int const segment_end,
                                  arma::colvec const& theta) {
    double const lambda = std::exp(DotTail(solver, segment_end, theta));
    return ScaledTail(solver, segment_end,
                      lambda - solver->data_(segment_end, 0));
  }
  ...
  static arma::mat GetHessian(Solver* solver, unsigned int const segment_start,
                              ...
                              arma::colvec const& theta) {
    double const lambda = std::exp(DotTail(solver, segment_end, theta));
    return TailOuter(solver, segment_end, std::min(lambda, 1e10));
  }
};
