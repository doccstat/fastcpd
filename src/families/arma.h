  ...
  static void GetNllPelt(T* instance, unsigned int const segment_start,
    ...
    try {
      Rcpp::List out = arima(
          ...
          Rcpp::Named("order") =
              Rcpp::NumericVector::create(instance->order_(0), 0, instance->order_(1)),
          Rcpp::Named("method") = "ML", Rcpp::Named("include.mean") = false);

      instance->result_coefficients_ = arma::zeros<arma::colvec>(arma::sum(instance->order_) + 1);
      instance->result_coefficients_.rows(0, arma::sum(instance->order_) - 1) =
          Rcpp::as<arma::colvec>(out["coef"]);
      instance->result_coefficients_(arma::sum(instance->order_)) = Rcpp::as<double>(out["sigma2"]);
      ...
      instance->result_coefficients_(arma::sum(instance->order_)) = arma::var(data_segment.col(0));
    }
#endif
  }
