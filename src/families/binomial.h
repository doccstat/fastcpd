...
struct BinomialFamily : BaseFamily {
  ...
  static void CreateSenParameters(Solver* solver) {
    ...
    double const prob = Logistic(DotTail(solver, 0, solver->coefficients_.col(0)));
    solver->hessian_.slice(0) = TailOuter(solver, 0, prob * (1.0 - prob));
  }
  ...
  static void UpdateSenParameters(Solver* solver) {
    ...
    double const prob = Logistic(DotTail(solver, solver->t - 1, coef_add));
    arma::mat hessian_new =
        TailOuter(solver, solver->t - 1, prob * (1.0 - prob));
  ...
  static double GetNllSen(Solver* solver, unsigned int const segment_start,
    ...
    for (unsigned int row = segment_start; row <= segment_end; row++) {
      ...
      cost += -solver->data_(row, 0) * eta + Log1pExp(eta);
    }
    return cost;
  }
  ...
  static arma::colvec GetGradient(Solver* solver,
                                  ...
                                  unsigned int const segment_end,
                                  arma::colvec const& theta) {
    double const prob = Logistic(DotTail(solver, segment_end, theta));
    return ScaledTail(solver, segment_end,
                      prob - solver->data_(segment_end, 0));
  }
  ...
  static arma::mat GetHessian(Solver* solver, unsigned int const segment_start,
                              ...
                              arma::colvec const& theta) {
    double const prob = Logistic(DotTail(solver, segment_end, theta));
    return TailOuter(solver, segment_end, (1.0 - prob) * prob);
  }
};
