...
context("get_hessian Unit Test") {
  test_that("binomal is correct for a two dimensional data") {
    const mat hessian =
        FastcpdTest::get_hessian_binomial({{1, 1, 0.2}}, 0, 0, {-0.5, 0.3});
    const mat expected_hessian = {{0.238'28, 0.047'656},
                                  {0.047'656, 0.009'531'2}};
    expect_true(norm(hessian - expected_hessian, "fro") < 0.000'001);
  }

  test_that("poisson is correct for a two dimensional data") {
    const mat hessian =
        FastcpdTest::get_hessian_poisson({{4, 1, 0.2}}, 0, 0, {-0.5, 0.3});
    const mat expected_hessian = {{0.644'036'4, 0.128'807},
                                  {0.128'807, 0.025'761'6}};
    expect_true(norm(hessian - expected_hessian, "fro") < 0.000'001);
  }
