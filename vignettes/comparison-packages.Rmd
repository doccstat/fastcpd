...
y <- c(
  x[1:300, ] %*% theta_0[1, ] + rnorm(300, 0, 10),
  x[301:700, ] %*% theta_0[2, ] + rnorm(400, 0, 10),
  x[701:n, ] %*% theta_0[3, ] + rnorm(300, 0, 10)
)
...
# Logistic regression
x <- matrix(rnorm(300 * 5, 0, 1), ncol = 5)
theta <- rbind(rnorm(5, 0, 1), rnorm(5, 2, 1))
y <- c(
  rbinom(175, 1, 1 / (1 + exp(-x[1:175, ] %*% theta[1, ]))),
  rbinom(125, 1, 1 / (1 + exp(-x[176:300, ] %*% theta[2, ])))
)
...
The true change point is 175.

```{r logistic-regression-fastcpd}
fastcpd::fastcpd.binomial(
  binomial_data, cost_adjustment = NULL, r.progress = FALSE
)@cp_set
```
...
```{r lasso-fastcpd}
fastcpd::fastcpd.lasso(
  lasso_data, beta = "BIC", cost_adjustment = "BIC", r.progress = FALSE
)@cp_set
```
