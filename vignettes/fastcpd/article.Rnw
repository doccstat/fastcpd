...
kfunc <- function(k, segment_length) {
  k - 1
  # min(max(k, ceiling(1000 / segment_length)), 10)
}
...
rng <- RNGseq(experiment_num * nrow(experiment_setup), 1)

cl <- parallel::makeCluster(parallel::detectCores(), outfile = paste0("log/ex.log"))
doParallel::registerDoParallel(cl)
experiment_result <- foreach::foreach(experiment_setup_index = seq_len(nrow(experiment_setup)), .packages = c("fastglm", "fastcpd", "fossil")) %:%
  foreach::foreach(experiment_index = seq_len(5), rrng = rng[(experiment_setup_index - 1) * experiment_num + seq_len(5)], .combine = "rbind") %dopar% {
...
get_result <- function(n, p, change_points_count, sgd_k) {
  experiment_setup_index <- which(
    experiment_setup$n == n &
    experiment_setup$p == p &
    experiment_setup$change_points_count == change_points_count &
    experiment_setup$sgd_k == sgd_k
  )
  paste0(format(round(mean(experiment_result[[experiment_setup_index]][, 1]), 2), nsmall = 2), ", ", format(round(mean(experiment_result[[experiment_setup_index]][, 2]), 2), nsmall = 2), "s")
}
@
...
rand_gd <- time_gd <- rep(list(rep(NA, experiment_num)), nrow(experiment_setup))
rng <- RNGseq(experiment_num * nrow(experiment_setup), 2)
...
% SGD PELT $k = 1$ & \Sexpr{summary(benchmarked)[1, 5]} & \Sexpr{fastcpd(data, (p + 1) * log(n) / 2, k = function(x) 0, family = "binomial")@cp_set} \\
% SGD PELT $k = 3$ & \Sexpr{summary(benchmarked)[2, 5]} & \Sexpr{fastcpd(data, (p + 1) * log(n) / 2, k = function(x) 2, family = "binomial")@cp_set} \\
% SGD PELT $k = 5$ & \Sexpr{summary(benchmarked)[3, 5]} & \Sexpr{fastcpd(data, (p + 1) * log(n) / 2, k = function(x) 4, family = "binomial")@cp_set} \\
% Vanilla PELT     & \Sexpr{summary(benchmarked)[4, 5]} & \Sexpr{CP_vanilla_binomial(data[, c(1:p + 1, 1)], (p + 1) * log(n) / 2)$cp} \\ \hline
% \end{tabular}
...
CP_vanilla_lasso <- function(data, beta, B=10, cost=cost_lasso, family="gaussian")
{
  n <- dim(data)[1]
  p <- dim(data)[2] - 1
...
rand_gd <- time_gd <- rep(list(rep(NA, experiment_num)), nrow(experiment_setup))
rng <- RNGseq(experiment_num * nrow(experiment_setup), 4)
...
%
<<quantile regression correct model, fig=TRUE, height=5.2, width=7>>=
...
%
<<mean shift model with custom cost function>>=
