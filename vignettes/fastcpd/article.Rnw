...
experiment_result <- foreach::foreach(experiment_setup_index = seq_len(nrow(experiment_setup)), .packages = c("fastglm", "fastcpd", "fossil", "glmnet")) %:%
  foreach::foreach(experiment_index = seq_len(5), rrng = rng[(experiment_setup$data_setup_index[experiment_setup_index] - 1) * experiment_num + seq_len(5)], .combine = "rbind") %dopar% {
