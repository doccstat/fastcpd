...
```{r Linear regression with one-dimensional covariate}
library(fastcpd)
set.seed(1)
p <- 1
x <- mvtnorm::rmvnorm(300, rep(0, p), diag(p))
...
```{r Custom cost function: logistic regression}
library(fastcpd)
set.seed(1)
p <- 5
...
```{r Custom cost function: mean shift}
library(fastcpd)
set.seed(1)
p <- 1
data <- rbind(
...
```{r Custom cost function: variance change}
library(fastcpd)
set.seed(1)
p <- 1
...
```{r Custom cost function: Huber loss}
library(fastcpd)
set.seed(1)
n <- 400 + 300 + 500
p <- 5
