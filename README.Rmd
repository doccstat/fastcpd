...
<summary>I countered problems related to gfortran on mac!</summary>

The package should be able to install on mac without any problems if all the
dependencies are installed. However, if you encountered problems related to
gfortran, it might be because `RcppArmadillo` is not installed previously. Try
this [stackoverflow solution](https://stackoverflow.com/a/72997915) if you have
...
```{r custom_cost_function_logistic_regression}
library(fastcpd)
set.seed(1)
p <- 5
...
```{r custom_cost_function_mean_shift}
library(fastcpd)
set.seed(1)
p <- 1
data <- rbind(
...
```{r custom_cost_function_multivariate_mean_shift}
library(fastcpd)
set.seed(1)
p <- 3
data <- rbind(
...
```{r custom_cost_function_mean_shift_and_variance_change}
library(fastcpd)
set.seed(1)
p <- 1
data <- rbind.data.frame(
...
```{r custom_cost_function_huber_loss}
library(fastcpd)
set.seed(1)
n <- 400 + 300 + 500
p <- 5
