...
```{r poisson regression, eval = FALSE}
library(fastcpd)
set.seed(1)
p <- 3
...
```{r penalized linear regression, eval = FALSE}

```

### Custom cost function: mean shift

```{r custom cost function: mean shift, eval = FALSE}
library(fastcpd)
set.seed(1)
p <- 1
data <- rbind(
...
```{r custom cost function: variance change, eval = FALSE}
library(fastcpd)
set.seed(1)
p <- 1
...
```{r custom cost function: mean shift and variance change, eval = FALSE}
library(fastcpd)
set.seed(1)
p <- 1
data <- rbind.data.frame(
...
```{r custom cost function: Huber loss, eval = FALSE}
library(fastcpd)
set.seed(1)
n <- 500 + 700 + 500
p <- 8
...
* examples in roxygen docs and multivariate custom cost functions, pkgdown references
